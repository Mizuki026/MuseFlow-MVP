...
- **真实调用状态**：已按用户授权执行恰好一次北京地域 `wan2.6-t2i` 异步提交，使用 `n=1`、`size=1280*1280`、`prompt_extend=false`。提交返回 HTTP 404，脱敏响应未提供可用 Provider 错误码；未获得 `task_id`，因此没有轮询、下载或重试，也没有可归因于本次请求的生成费用。该结果证明当前环境下这次实际提交没有进入可轮询任务流程，但不能单独区分工作空间端点配置、API 路径或账号侧资源问题。
...
**待验证**：上述凭据与 API Host 组合是否对应文档所需的北京工作空间 API 路径；本次真实提交返回 HTTP 404，尚未证明账号已进入可轮询任务流程。
...
- 已执行恰好一次脱敏的真实异步提交：HTTP 404，未返回可用 `task_id`；按停止条件立即结束，没有轮询、下载或重试，也没有产生可确认的图片费用。
- 当前运行环境此前对 DashScope 公共图像生成端点执行过不带凭据、不会生成图片的 HEAD 连通性探测，返回 HTTP 401；这只证明 DNS、TLS 和 HTTP 可达，不证明工作空间、账号、模型或计费可用。

### 10.2 尚未验证

- 这次 HTTP 404 的具体原因，以及 `DASHSCOPE_API_HOST` 与北京工作空间 API 路径的正确组合。
...
1. 定位并修正本次真实提交返回 HTTP 404 的 API Host、路径或工作空间配置问题；
2. 在修正后完成一次脱敏的真实成功调用记录，并验证提交、轮询、结果下载、像素尺寸和主机；

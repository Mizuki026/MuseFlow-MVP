...
- **真实调用状态**：首次授权提交因 Host/路径配置返回 HTTP 404，未获得 `task_id`。用户更正环境变量后，第二次授权的真实异步提交使用 `n=1`、`size=1280*1280`、`prompt_extend=false`，已到达北京 Workspace，但 Provider 返回 HTTP 401、错误码 `InvalidApiKey`；按停止条件立即结束，没有轮询、下载或重试，也没有可确认的图片费用。第二次请求前的本地预检发现 `DASHSCOPE_API_HOST` 带有 `/api/v1`，探针仅在本地规范化该已知前缀后提交。
- **阶段结论**：官方资料调查已完成，真实探针已执行但阶段 0 **尚未通过**。在 404 原因得到修正并完成一次成功提交、轮询和下载前，不能开始第 1 个实现窗口。
...
**待验证**：北京 Workspace Host 已能到达 Provider，但目标环境中的 API Key 是否为有效、未过期且与北京地域/该 Workspace 匹配；第二次真实提交返回 `InvalidApiKey`，尚未进入可轮询任务流程。
...
**待验证**：真实北京账号返回的精确主机、是否重定向、Content-Type、Content-Length、PNG 文件签名、图片实际尺寸以及 URL 在短时间内的访问行为。无需等待 24 小时证明过期；过期时长可直接依据官方契约。
...
- Provider 返回 `InvalidApiKey` 的具体账号原因：确认 API Key 未过期、未带引号或空白、属于北京地域并绑定目标 Workspace；必要时从北京业务空间重新创建/导出 Key。
- Wan 北京账号的真实成功创建、轮询、任务状态、耗时、错误 JSON、限流响应和结果下载行为。
- Wan 实际返回的精确下载主机、重定向、响应头、文件大小、签名和像素尺寸。
- Replicate 在中国大陆的网络、账号与支付可用性，FLUX 输出的精确尺寸和安全拒绝结构。
- OpenAI 目标账号的地区与模型权限、固定请求实际成本、同步超时后的真实错误结构。
- 429 和 5xx 的真实 `Retry-After`、退避效果和账号特定限额。

### 10.3 阶段 0 判定

**阶段 0 当前未通过，第 1 个实现窗口现在不应开始。** 精确阻塞项是：

1. 修正并验证 Provider 报告的 `InvalidApiKey`，确保 Key 与北京 Workspace 匹配；
2. 在获得新的单次付费授权后完成一次脱敏的真实成功调用记录，并验证提交、轮询、结果下载、像素尺寸和主机；
3. 对不会主动制造的 429、5xx 和超时分支完成本地协议模拟验证，并在文档中明确其证据层级。

完成这些步骤后，若结果与官方契约一致，可将阶段 0 标记为通过并进入第 1 个实现窗口。若北京账号不可用，应按实际部署地区重新选择候选，而不是把公开资料结论当作实测通过。
